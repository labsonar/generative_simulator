...
def main(args) -> None:
    ...
    for classe in classes:
        subprocess.run(["python","test/model_training.py", "--max_steps", args.max_steps, f"./model/{classe}", f"/data/4classes/{classe}"], check=True)
        subprocess.run(["python","test/model_inference.py", "-t", args.n_samples, "-o", f"./audio/{classe}", f"./model/{classe}/weights.pt"], check = True)
    subprocess.run(["python","test/spectral_plot.py","./audio/A", './audio/B', './audio/C', './audio/D', '/data/4classes/A', '/data/4classes/B', '/data/4classes/C', '/data/4classes/D'])
